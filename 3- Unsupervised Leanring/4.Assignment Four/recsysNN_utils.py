""" Utilities for RecSysNN assignment """
...
def load_data():
    """ called to load prepared data for the lab """
...
def pprint_train(x_train, features, vs, u_s, maxcount=5, user=True):
    ...
    table = tabulate.tabulate(disp, tablefmt='html', headers="first-row", floatfmt=flist, numalign='center')
    return table


def split_str(input_features, smax):
    """ split the feature name strings to tables fit """
    output_features = []
    for s in input_features:
        if not ' ' in s:  # skip string that already have a space
            if len(s) > smax:
                mid = int(len(s)/2)
                s = s[:mid] + " " + s[mid:]
        output_features.append(s)
...
def print_pred_movies(y_p, item, movie_dict, maxcount=10):
    ...
    table = tabulate.tabulate(disp, tablefmt='html', headers="first-row")
    return table
